...
def load_db_annotation(base_path, set_name):
    ...
    if set_name == 'training':
        ...
        if cfg.small_dataset:
            k_path = os.path.join(base_path, '%s_small_K.json' % set_name)
            xyz_path = os.path.join(base_path, '%s_small_xyz.json' % set_name)
            scale_path = os.path.join(base_path, '%s_small_scale.json' % set_name)
            vert_path = os.path.join(base_path, '%s_small_verts.json' % set_name)
        ...
        """
        if cfg.small_dataset:
            small_len = int(len(K_list) / 100)
            K_list = K_list[:small_len]
            vert_list = vert_list[:small_len]
            xyz_list = xyz_list[:small_len]
            scale_list = scale_list[:small_len]

            k_path_small = os.path.join(base_path, '%s_small_K.json' % set_name)
            xyz_path_small = os.path.join(base_path, '%s_small_xyz.json' % set_name)
            scale_path_small = os.path.join(base_path, '%s_small_scale.json' % set_name)
            vert_path_small = os.path.join(base_path, '%s_small_verts.json' % set_name)

            json_save(k_path_small, K_list)
            json_save(xyz_path_small, xyz_list)
            json_save(scale_path_small, scale_list)
            json_save(vert_path_small, vert_list)
        """

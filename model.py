...
class SAR_TE(nn.Module):
    def __init__(self):
        super(SAR_TE, self).__init__()

        self.mode = cfg.mode_addPos
        mano = MANO()
        backbone = models.__dict__[cfg.backbone](pretrained=True)
        self.extract_mid = nn.Sequential(backbone.conv1, backbone.bn1, backbone.relu,
                                         backbone.maxpool, backbone.layer1, backbone.layer2)
        ...
        for i in range(cfg.num_stage):
            ...
            self.gbbmr.append(GBBMR(cfg.num_FMs*cfg.feature_size+3, cfg.num_vert, cfg.num_joint, cfg.heatmap_size))
            if i > 0:
                if self.mode is 'Back':
                    self.fuse.append(nn.Conv2d(channel // 4 + cfg.num_joint * 2 + 32, channel // 4, 1))
                else:
                    self.fuse.append(nn.Conv2d(channel // 4 + cfg.num_joint * 2, channel // 4, 1))
    ...
    def forward(self, input, target=None):
        x = input['img'].to(cfg.device)     # (batch, 3, 256, 256)
        prev = input['prev_heatmap'].to(cfg.device)     # (batch, 1, 64, 64)
        outs = {'coords': []}

        lhms = []
        dms = []
        feat_mid = self.extract_mid(x)        # (batch, 128, 32, 32)
        feat_prev = self.extract_prev(prev)     # (batch, 32, 32, 32)

        feat_sim = self.extract_sim(torch.cat((feat_mid, feat_prev), dim=1))        # (batch, 32, 32, 32)
        weight_sim = self.extract_weight(feat_sim)

        feat_sim = torch.mul(feat_sim, weight_sim)

        weight_sim = torch.squeeze(weight_sim)
        ...
        for i in range(cfg.num_stage):
            ...
            if self.mode is 'Front':
                feat = self.latent_encoder(torch.cat((feat, feat_sim), dim=1))    # (batch, 128+32, 32, 32) to (batch, 128, 32, 32)
        ...
        if self.training:
            loss = {}
            mesh_pose_uvd = target['mesh_pose_uvd'].to(cfg.device)
            aug_weight = target['aug_weight'].to(cfg.device)
            # if aug_weight is 0, weight_sim need to be 1
            # if aug_weight is 1~5, weight_sim need to be small
            aug_weight = 1 / (aug_weight + 1.)
            ...
            for i in range(cfg.num_stage):
                ...
                loss['edge_{}'.format(i)] = \
                    self.edge_loss(outs['coords'][i][:, :cfg.num_vert], mesh_pose_uvd[:, :cfg.num_vert]).mean()
            loss['aug_weight_{}'.format(i)] = self.coord_loss(weight_sim, aug_weight)
...
def get_model():
    if cfg.model_name is 'SAR':
        return SAR()
    if cfg.model_name is 'SAR_TE':
        return SAR_TE()
